...
export async function payMpesa(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    const accessToken = (tokenResponse as { access_token: string })
      .access_token;
    ...
    try {
      ...
      const payment = await prisma.payment.create({
        data: {
          ...
          failureReason: stkPushResponse.data.ResponseDescription, // Store the response description
...
export async function mpesaWebhook(
  ...
  try {
    ...
    if (ResultCode === 0) {
      ...
      await prisma.payment.updateMany({
        where: { transactionId: CheckoutRequestID },
        data: { paymentStatus: "failed", failureReason: ResultDesc },
      });
